...
function onSpeak(player, type, message)
    ...
    if player:getCondition(CONDITION_CHANNELMUTEDTICKS, CONDITIONID_DEFAULT, CHANNEL_GLOBAL) then
        ...
        player:sendCancelMessage("You are muted from the Search Guild Channel for using it inappropriately." .. remainingTime)
        return false
	end
            ...
            if target then
                if playerAccountType > target:getAccountType() then
                    ...
                    if not target:getCondition(CONDITION_CHANNELMUTEDTICKS, CONDITIONID_DEFAULT, CHANNEL_GLOBAL) then
                        ...
                        sendChannelMessage(CHANNEL_GLOBAL, TALKTYPE_CHANNEL_R1, target:getName() .. " has been muted by " .. player:getName() .. " for using Search Guild Channel inappropriately. Mute will last " .. muteDuration .. " minutes.")
                    else
