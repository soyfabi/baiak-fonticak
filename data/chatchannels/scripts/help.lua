...
function onSpeak(player, type, message)
    ...
    if player:getCondition(CONDITION_CHANNELMUTEDTICKS, CONDITIONID_DEFAULT, CHANNEL_HELP) then
        local muteCondition = player:getCondition(CONDITION_CHANNELMUTEDTICKS, CONDITIONID_DEFAULT, CHANNEL_HELP)
        ...
        player:sendCancelMessage("You are muted from the Help Channel for using it inappropriately." .. remainingTime)
        return false
	end
            ...
            if target then
                if playerAccountType > target:getAccountType() then
                    ...
                    if not target:getCondition(CONDITION_CHANNELMUTEDTICKS, CONDITIONID_DEFAULT, CHANNEL_HELP) then
                        ...
                        sendChannelMessage(CHANNEL_HELP, TALKTYPE_CHANNEL_R1, target:getName() .. " has been muted by " .. player:getName() .. " for using Help Channel inappropriately. Mute will last " .. muteDuration .. " minutes.")
                    else
